...
扫描器最多向前看 4 个字符，以此为根据选择特定的扫描方法或者 Token。之所以选择 4 个字符，是因为这是[ JavaScript 中歧义字符序列](#id1)的最大长度。
...
`AdvanceUntil` 对于加快可能需要处理大量字符的扫描函数特别有用。我们使用它来加速前面已经提到的标识符，但也包括[字符串](#id2)和注释。
